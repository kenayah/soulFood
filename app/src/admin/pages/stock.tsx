            ...
            {ingredients.map((ing) => {
              ...
              return <>
                <tr class={isLow ? "bg-error/10" : ""}>
                  <td>{ing.name}{!categoryFilter && <><br/><span class="text-xs text-base-content/60 max-md:hidden">{ing.category_name ?? "—"}</span></>}</td>
                  <td>{ing.unit}</td>
                ...
                {!categoryFilter && <tr class="md:hidden"><td colspan={10} class="text-xs text-base-content/60 pb-1 pt-0">Category: {ing.category_name ?? "—"}</td></tr>}
